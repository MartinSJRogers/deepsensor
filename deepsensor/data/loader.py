    ...
    def generate_tasks(
        ...
        **kwargs,
        ...
        if patch_strategy is None:
            ...
            for date in dates:
                for i in bbox:
                    tasks = [self.task_generation(date, bbox[i, :],**kwargs)]
                """
                tasks.extend( ## Extent, as in continually add to the end of the list of Tasks.
                    [
                        self.task_generation(date, **new_kwargs)
                        for _ in range(num_samples_per_date) ## Need to think of the equivalent here, as I don't have a number. 
                    ]
                )
                """
