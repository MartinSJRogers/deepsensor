...
for date in pd.date_range(train_start_date, train_end_date):
    tasks = task_loader(date, context_sampling="all", target_sampling="all",
...
test_task = task_loader(test_date, context_sampling="all", target_sampling="all",
                        patch_strategy="sliding", patch_size=patch_size, stride=stride_size)
prediction = model.predict_patch(test_task, X_t=amsr_raw_ds, data_processor = data_processor)